...
{ 
 if((esp_timer_get_time() - prev_tim) >= sampleTime) //Running the PID at a specific frequency
    ...
    gpio_set_level(LEFT_MOTOR_ENABLE, 1);       // enabke the motors
...
{  
    ...
    gpio_set_level(LEFT_MOTOR_ENABLE, 1);       // enabke the motors
    gpio_set_level(RIGHT_MOTOR_ENABLE, 1);
    gpio_set_level(LEFT_MOTOR_DIRECTION, 1);    // IMPLEMENT DIFFERENTIAL
...
{   //ESP_LOGI(TAG,"right");
    ...
    gpio_set_level(LEFT_MOTOR_ENABLE, 1);   // enabke the motors
    gpio_set_level(RIGHT_MOTOR_ENABLE, 1);
    gpio_set_level(LEFT_MOTOR_DIRECTION, 0);    //IMPLEMENT DIFFERENTIAL
    gpio_set_level(RIGHT_MOTOR_DIRECTION, 1);
...
void move_stop()
{   gpio_set_level(LEFT_MOTOR_ENABLE, 0);   // Disable the motor driver channel which automatically turns off the motor
