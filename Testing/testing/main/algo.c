...
void init_gpio()
{
    ...
    gpio_set_direction(LEFT_MOTOR_DIR1, GPIO_MODE_OUTPUT);
    gpio_set_direction(LEFT_MOTOR_DIR2, GPIO_MODE_OUTPUT);
    
    gpio_set_direction(RIGHT_MOTOR_DIR1, GPIO_MODE_OUTPUT);
    gpio_set_direction(RIGHT_MOTOR_DIR2, GPIO_MODE_OUTPUT);
...
void init_pwm()
{
    ...
    motorL.gpio_num = LEFT_MOTOR_PWM;			//the GPIO pin connected to Left motor controller PWM port
    ...
    motorR.gpio_num = RIGHT_MOTOR_PWM;			//the GPIO pin connected to Right motor controller PWM port
...
{   //ESP_LOGI(TAG,"FORWARD :CURRENT(x,y):%f %f path point(x,y): %f %f ",current_point[0],current_point[1],stop_point[0],stop_point[1]);
   /* if((esp_timer_get_time() - prev_tim) >= sampleTime) //Running the PID at a specific frequency
    ...
    gpio_set_level(LEFT_MOTOR_DIR1, 1);    //BOTH IN SAME DIRECTION
    gpio_set_level(LEFT_MOTOR_DIR2, 0);

    gpio_set_level(RIGHT_MOTOR_DIR1, 1);    //BOTH IN SAME DIRECTION
    gpio_set_level(RIGHT_MOTOR_DIR2, 0);
...
{    //ESP_LOGI(TAG,"FORWARD :CURRENT(x,y):%f %f path point(x,y): %f %f ",current_point[0],current_point[1],stop_point[0],stop_point[1]);   
 /*if((esp_timer_get_time() - prev_tim) >= sampleTime){
    ...
    gpio_set_level(RIGHT_MOTOR_DIR1, 0);    //BOTH IN SAME DIRECTION
    gpio_set_level(RIGHT_MOTOR_DIR2, 1);
...
/*if((esp_timer_get_time() - prev_tim) >= sampleTime){
    ...
    gpio_set_level(RIGHT_MOTOR_DIRECTION, 1);*/
    gpio_set_level(LEFT_MOTOR_DIR1, 0);    //BOTH IN SAME DIRECTION
    gpio_set_level(LEFT_MOTOR_DIR2, 1);
