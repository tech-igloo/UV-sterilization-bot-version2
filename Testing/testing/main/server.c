...
esp_err_t handle_pause(httpd_req_t *req)
{
    ...
    free(resp);
    ESP_LOGI(TAG, "Record Flag: %d", record_flag);
    if(record_flag == 1 && det != 's') //If recording is ongoing, then store the direction in which it was travelling and the time duration for which it had travelled in that direction
    {
        FILE* f = fopen("/spiffs/paths.txt", "a");
        if (f == NULL) {
            ESP_LOGE(TAG, "Failed to open file for writing");
...
/*HTML Code for displaying the home page "/"3498db */
char* default_page()
{
    ...
    strcat(ptr, ".button {display: block;width: 100px;background-color: #2000db;border: none;color: white;padding: 13px 30px;text-decoration: none;font-size: 25px;margin: 0px auto 35px;cursor: pointer;border-radius: 4px;}\n");
